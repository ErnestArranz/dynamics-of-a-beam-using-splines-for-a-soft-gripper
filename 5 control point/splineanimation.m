...
% q(:,1) -> x1, q(:,2) -> x2, q(:,3) -> y2, q(:,4) -> x3, q(:,5) -> y3, q(:,6) -> x4, q(:,7) -> y4
% Vector de tiempo t
...
x_min = min([X(:,1)', X(:,2)', X(:,4)', X(:,6)']); % Mínimo valor de x (incluyendo todos los puntos)
x_max = max([X(:,1)', X(:,2)', X(:,4)', X(:,6)']); % Máximo valor de x (incluyendo todos los puntos)
y_min = min([X(:,3)', X(:,5)', X(:,7)']);         % Mínimo valor de y (incluyendo todos los puntos)
y_max = max([X(:,3)', X(:,5)', X(:,7)']);         % Máximo valor de y (incluyendo todos los puntos)
...
traj_P2_x = []; traj_P2_y = [];
traj_P3_x = []; traj_P3_y = [];
traj_P4_x = []; traj_P4_y = []; % Añadido para el punto P4

n=100;

% Create a VideoWriter object
videoFile = 'quartic_spline_animation_force_001.avi'; % Specify the output video file name
...
for k = 1:n:length(t) % skip frames to make animation faster
    ...
    P4 = [X(k, 6), X(k, 7)]; % Punto final (x4, y4) - nuevo punto
    ...
    traj_P4_x = [traj_P4_x, P4(1)]; traj_P4_y = [traj_P4_y, P4(2)]; % Añadido para P4
    ...
    plot(traj_P4_x, traj_P4_y, 'c-', 'LineWidth', 1.5, 'DisplayName', 'Trayectoria P4'); % Añadido para P4
    ...
    plot(P4(1), P4(2), 'co', 'MarkerSize', 8, 'MarkerFaceColor', 'c', 'DisplayName', 'Punto P4'); % Añadido para P4
    ...
    plot([P3(1), P4(1)], [P3(2), P4(2)], 'k:', 'LineWidth', 1); % Añadido para P4

    % Calcular y dibujar la spline completa (cuártica)
    pu_x = zeros(size(u));
    pu_y = zeros(size(u));
    for i = 1:length(u)
        % Calcular los coeficientes de Bernstein para curva cuártica
        b0 = (1 - u(i))^4;
        b1 = 4 * u(i) * (1 - u(i))^3;
        b2 = 6 * u(i)^2 * (1 - u(i))^2;
        b3 = 4 * u(i)^3 * (1 - u(i));
        ...
        pu_x(i) = b0 * P0(1) + b1 * P1(1) + b2 * P2(1) + b3 * P3(1) + b4 * P4(1);
        pu_y(i) = b0 * P0(2) + b1 * P1(2) + b2 * P2(2) + b3 * P3(2) + b4 * P4(2);
    end
    ...
    legend({'P0 trajectory', 'P1 trajectory', 'P2 trajectory', 'P3 trajectory', 'P4 trajectory', ...
           ' P0', ' P1', ' P2', ' P3', ' P4', 'Spline'}, ...
           'Location', 'northeast');
...
P4_initial = [X(1, 6), X(1, 7)]; % Añadido para P4
...
P4_final = [X(end, 6), X(end, 7)]; % Añadido para P4
...
for i = 1:length(u_plot)
    % Coeficientes para curva cuártica
    b0 = (1 - u_plot(i))^4;
    b1 = 4 * u_plot(i) * (1 - u_plot(i))^3;
    b2 = 6 * u_plot(i)^2 * (1 - u_plot(i))^2;
    b3 = 4 * u_plot(i)^3 * (1 - u_plot(i));
    b4 = u_plot(i)^4;
    
    % Initial spline
    spline_x_initial(i) = b0 * P0_initial(1) + b1 * P1_initial(1) + b2 * P2_initial(1) + b3 * P3_initial(1) + b4 * P4_initial(1);
    spline_y_initial(i) = b0 * P0_initial(2) + b1 * P1_initial(2) + b2 * P2_initial(2) + b3 * P3_initial(2) + b4 * P4_initial(2);
    
    % Final spline
    spline_x_final(i) = b0 * P0_final(1) + b1 * P1_final(1) + b2 * P2_final(1) + b3 * P3_final(1) + b4 * P4_final(1);
    spline_y_final(i) = b0 * P0_final(2) + b1 * P1_final(2) + b2 * P2_final(2) + b3 * P3_final(2) + b4 * P4_final(2);
end
