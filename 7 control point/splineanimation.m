...
% q(:,1) -> x1, q(:,2) -> x2, q(:,3) -> y2, q(:,4) -> x3, q(:,5) -> y3, q(:,6) -> x4, q(:,7) -> y4, q(:,8) -> x5, q(:,9) -> y5, q(:,10) -> x6, q(:,11) -> y6
% Vector de tiempo t
...
x_min = min([X(:,1)', X(:,2)', X(:,4)', X(:,6)', X(:,8)', X(:,10)']); % Mínimo valor de x (incluyendo todos los puntos)
x_max = max([X(:,1)', X(:,2)', X(:,4)', X(:,6)', X(:,8)', X(:,10)']); % Máximo valor de x (incluyendo todos los puntos)
y_min = min([X(:,3)', X(:,5)', X(:,7)', X(:,9)', X(:,11)']);         % Mínimo valor de y (incluyendo todos los puntos)
y_max = max([X(:,3)', X(:,5)', X(:,7)', X(:,9)', X(:,11)']);         % Máximo valor de y (incluyendo todos los puntos)
...
traj_P4_x = []; traj_P4_y = [];
traj_P5_x = []; traj_P5_y = [];
traj_P6_x = []; traj_P6_y = []; % Añadido para el punto P6

n=100;

% Create a VideoWriter object
videoFile = 'sextic_spline_animation_force_001.avi'; % Specify the output video file name
...
for k = 1:n:length(t) % skip frames to make animation faster
    ...
    P5 = [X(k, 8), X(k, 9)]; % Punto de control (x5, y5)
    P6 = [X(k, 10), X(k, 11)]; % Punto final (x6, y6) - nuevo punto
    ...
    traj_P5_x = [traj_P5_x, P5(1)]; traj_P5_y = [traj_P5_y, P5(2)];
    traj_P6_x = [traj_P6_x, P6(1)]; traj_P6_y = [traj_P6_y, P6(2)]; % Añadido para P6
    ...
    plot(traj_P5_x, traj_P5_y, 'y-', 'LineWidth', 1.5, 'DisplayName', 'Trayectoria P5');
    plot(traj_P6_x, traj_P6_y, 'k-', 'LineWidth', 1.5, 'DisplayName', 'Trayectoria P6'); % Añadido para P6
    ...
    plot(P5(1), P5(2), 'yo', 'MarkerSize', 8, 'MarkerFaceColor', 'y', 'DisplayName', 'Punto P5');
    plot(P6(1), P6(2), 'ko', 'MarkerSize', 8, 'MarkerFaceColor', 'k', 'DisplayName', 'Punto P6'); % Añadido para P6
    ...
    plot([P4(1), P5(1)], [P4(2), P5(2)], 'k:', 'LineWidth', 1);
    plot([P5(1), P6(1)], [P5(2), P6(2)], 'k:', 'LineWidth', 1); % Añadido para P6

    % Calcular y dibujar la spline completa (sextic)
    pu_x = zeros(size(u));
    pu_y = zeros(size(u));
    for i = 1:length(u)
        % Calcular los coeficientes de Bernstein para curva sextic (grado 6)
        b0 = (1 - u(i))^6;
        b1 = 6 * u(i) * (1 - u(i))^5;
        b2 = 15 * u(i)^2 * (1 - u(i))^4;
        b3 = 20 * u(i)^3 * (1 - u(i))^3;
        ...
        pu_x(i) = b0 * P0(1) + b1 * P1(1) + b2 * P2(1) + b3 * P3(1) + b4 * P4(1) + b5 * P5(1) + b6 * P6(1);
        pu_y(i) = b0 * P0(2) + b1 * P1(2) + b2 * P2(2) + b3 * P3(2) + b4 * P4(2) + b5 * P5(2) + b6 * P6(2);
    end
    ...
    legend({'P0 trajectory', 'P1 trajectory', 'P2 trajectory', 'P3 trajectory', 'P4 trajectory', 'P5 trajectory', 'P6 trajectory', ...
           ' P0', ' P1', ' P2', ' P3', ' P4', ' P5', ' P6', 'Spline'}, ...
           'Location', 'northeast');
...
P5_initial = [X(1, 8), X(1, 9)];
P6_initial = [X(1, 10), X(1, 11)]; % Añadido para P6
...
P5_final = [X(end, 8), X(end, 9)];
P6_final = [X(end, 10), X(end, 11)]; % Añadido para P6
...
for i = 1:length(u_plot)
    % Coeficientes para curva sextic (grado 6)
    b0 = (1 - u_plot(i))^6;
    b1 = 6 * u_plot(i) * (1 - u_plot(i))^5;
    b2 = 15 * u_plot(i)^2 * (1 - u_plot(i))^4;
    b3 = 20 * u_plot(i)^3 * (1 - u_plot(i))^3;
    b4 = 15 * u_plot(i)^4 * (1 - u_plot(i))^2;
    b5 = 6 * u_plot(i)^5 * (1 - u_plot(i));
    b6 = u_plot(i)^6;
    
    % Initial spline
    spline_x_initial(i) = b0 * P0_initial(1) + b1 * P1_initial(1) + b2 * P2_initial(1) + b3 * P3_initial(1) + b4 * P4_initial(1) + b5 * P5_initial(1) + b6 * P6_initial(1);
    spline_y_initial(i) = b0 * P0_initial(2) + b1 * P1_initial(2) + b2 * P2_initial(2) + b3 * P3_initial(2) + b4 * P4_initial(2) + b5 * P5_initial(2) + b6 * P6_initial(2);
    
    % Final spline
    spline_x_final(i) = b0 * P0_final(1) + b1 * P1_final(1) + b2 * P2_final(1) + b3 * P3_final(1) + b4 * P4_final(1) + b5 * P5_final(1) + b6 * P6_final(1);
    spline_y_final(i) = b0 * P0_final(2) + b1 * P1_final(2) + b2 * P2_final(2) + b3 * P3_final(2) + b4 * P4_final(2) + b5 * P5_final(2) + b6 * P6_final(2);
end
...
plot(P6_initial(1), P6_initial(2), 'ko', 'MarkerSize', 8, 'MarkerFaceColor', 'k', 'DisplayName', 'P6 (Initial)');
...
plot([P5_initial(1), P6_initial(1)], [P5_initial(2), P6_initial(2)], 'k:');
...
plot(P6_final(1), P6_final(2), 'ks', 'MarkerSize', 8, 'MarkerFaceColor', 'k', 'DisplayName', 'P6 (Final)');
...
plot([P5_final(1), P6_final(1)], [P5_final(2), P6_final(2)], 'k--');
